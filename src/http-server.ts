...
// HTTP server to handle JSON-RPC requests
const httpServer = http.createServer(async (req, res) => {
  // Enable CORS
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'content-type, accept, jsonrpc');
              ...
              tools: [
                {
                  ...
                  inputSchema: {
                    ...
                    properties: {
                      text: {
                        type: 'string',
                        description: 'The text to scan. Texts under 600 characters may produce unreliable results and should be avoided. Maximum 150 000 characters per request.',
                        minLength: 300,
                        maxLength: 150000
                      },
                      file: {
                        type: 'string',
                        description: 'A file to scan. If you supply a file, the API will scan the content of the file. The file must be in plain .pdf, .doc or .docx format.'
                      },
                      website: {
                        type: 'string',
                        description: 'A website URL to scan. If you supply a website, the API will fetch the content of the website and scan it.'
                      }
                    },
                    required: ['text']
                  }
                },
                {
                  ...
                  inputSchema: {
                    ...
                    properties: {
                      url: {
                        type: 'string',
                        description: 'Specifies the URL of the image to scan. The URL must be valid, publicly accessible, and point to an image in one of the following formats: JPG, JPEG, PNG, or WEBP.',
                        format: 'uri'
                      }
                    },
                    required: ['url']
                ...
                },
                {
                  ...
                  title: 'Plagiarism Detection',
                  description: 'Winston AI\'s plagiarism API is a powerful tool designed to check text for plagiarism by scouring the internet for similar content.',
                  inputSchema: {
                    type: 'object',
                    properties: {
                      text: {
                        type: 'string',
                        description: 'The text to be scanned. Each request must contain at least 100 characters and no more than 120,000 characters.',
                        minLength: 100,
                        maxLength: 120000
                      },
                      language: {
                      ...
                      country: {
                        type: 'string',
                        description: 'The country code of the country where the text was written. Default: us.',
                        default: 'us'
                      }
                    },
                    required: ['text']
          ...
          try {
            ...
            switch (name) {
              case 'ai-text-detection':
                toolResult = await handleAiTextDetection(args);
                break;
              case 'ai-image-detection':
                toolResult = await handleAiImageDetection(args);
                break;
              case 'plagiarism-detection':
                toolResult = await handlePlagiarismDetection(args);
                break;
              case 'text-compare':
                toolResult = await handleTextCompare(args);
                break;
              default:
                result = {
                  jsonrpc: '2.0',
...
async function handleAiTextDetection(args: any) {
  console.log('API Key check:', winstonAIClient.isApiKeyInvalid());
  console.log('API Key value:', process.env.WINSTONAI_API_KEY);
  ...
  try {
    ...
    console.error('Winston AI API Error:', {
      message: error.message,
      response: error.response?.data,
      status: error.response?.status,
      apiKey: process.env.WINSTONAI_API_KEY ? '***' + process.env.WINSTONAI_API_KEY.slice(-4) : 'undefined'
    });
    return {
      content: [
        {
          type: "text",
          text: `Winston AI API Error: ${error.response?.data?.message || error.message || 'Unknown error'}. Please check your API key and try again.`,
        },
      ],
    };
  }
...
async function handleAiImageDetection(args: any) {
  if (winstonAIClient.isApiKeyInvalid()) {
    return {
      content: [
        {
...
async function handlePlagiarismDetection(args: any) {
  if (winstonAIClient.isApiKeyInvalid()) {
    return {
      content: [
        {
...
async function handleTextCompare(args: any) {
  if (winstonAIClient.isApiKeyInvalid()) {
    return {
      content: [
        {
  ...
  if (!args.first_text || args.first_text.length === 0 || !args.second_text || args.second_text.length === 0) {
    return {
      content: [
        {
          type: "text",
...
httpServer.listen(PORT, () => {
  console.log(`Winston AI MCP Server is running on port ${PORT}`);
